		...
		$('.windows-main').on('mouseover','l', function(e) {
			...
			for (var i=0, il=strongParts.length; i<il; i++) {
				...
				if (verseid) { // lpnrmw
					...
					$('.' + verseid ).find('l[s^="' + langPrefix + strong + '"],l[s$="' + langPrefix + strong + '"],l[s=" ' + langPrefix + strong + ' "]').addClass('lemma-highlight');
				} else {
					//$('l[s*="' + strong + '"],l[s*="' + langPrefix + strong + '"]').addClass('lemma-highlight');
					$('l[s^="' + langPrefix + strong + '"],l[s$="' + langPrefix + strong + '"],l[s=" ' + langPrefix + strong + ' "]').addClass('lemma-highlight');
				}
