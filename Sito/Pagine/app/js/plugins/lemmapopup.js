...
var LemmaPopupPlugin = function(app) {
	...
	$('.windows-main').on('click','.BibleWindow l', function(e) {
		...
		lemmaPopup.body.html('Loading...');
			...
			lemmaPopup.body.removeClass('loading-indicator'); // moved from following subroutine loadStrongsData lpnrmw
		...
		sofia.ajax({
			...
			success: function(data) {
				...
				var html = '<div class="lemma-word">' +
								...
								' ' +
								'<span class="lemma-strongs" dir="ltr"> (' + langPrefix + strongsNumber + ')</span>' +
							'</div>';
                ...
                // lpnrmw prossime 5 righe
                var freq = 0;
                if (data.frequency.constructor == Array)
                    freq = data.frequency.length;
                else
                    freq = data.frequency;
				html += '<span class="lemma-findall" data-lemma="' + langPrefix + strongsNumber + '" data-textid="' + textid + '">' +
					//i18n.t('plugins.lemmapopup.findalloccurrences', {count: data.frequency}) + // lpnrmw
                    i18n.t('plugins.lemmapopup.findalloccurrences').replace('__count__', freq ) + // lpnrmw
				'</span>';

				//html += '<div class="lemma-outline">' + data.outline + '</div>'; // removed lpnrmw
                def = data.derivation + '</i><br />' + data.kjv_def + '<br /><br />' + data.strongs_def; // added lpnrmw
                if (typeof data.it_def != 'undefined') // added lpnrmw
                    def = def + '<br /><br />' + data.it_def; // added lpnrmw
				html += '<div class="lemma-outline"><i>' + def + '</div>'; // added lpnrmw

				// lemmaPopup.body.removeClass('loading-indicator'); // moved to preceding subroutine lpnrmw
			...
			error: function() {
				// lemmaPopup.body.html('Error loading ... ' + langPrefix + strongs[0] ); // lpnrmw
				lemmaPopup.body.html(langPrefix + strongsNumber ); // lpnrmw
			}
