...
var TextChooser = function() {
	...
	function filterVersions(e) {
		...
		if (text == 'qqq348rmw138c8d$') { // lpnrmw was ''
			renderTexts(list_data);
			//updateRecentlyUsed();
		} else {
			...
			for (var i=0, il=arrayOfTexts.length; i<il; i++) {
                ...
                var hasMatch = true;
                if (text.length>0) { // lpnrmw added so all texts are list if no filter applied
					hasMatch = 	textInfo.name.toLowerCase().indexOf(text) > -1 ||
								textInfo.abbr.toLowerCase().indexOf(text) > -1 ||
//								(textInfo.langNameEnglish && textInfo.langName.toLowerCase().indexOf(text) > -1) || // lpnrmw
								(textInfo.langName && textInfo.langName.toLowerCase().indexOf(text) > -1) || // lpnrmw
								(textInfo.langNameEnglish && textInfo.langNameEnglish.toLowerCase().indexOf(text) > -1);
                } // lpnrmw add to close the above if statement
			...
			for (var index in languages) {
				...
				if (text_type == 'bible' && (mode == 'none' || mode == 'languages' || (hasDefaultText && mode == 'default')) ) {
                    ...
                    */
                    ...
                    //lpnrmw
					var langName = textsInLang[0].langName,
						langNameItaliano = textsInLang[0].langNameItaliano;
					if (langNameItaliano != '' && langNameItaliano != undefined) {
						languageDisplayTitle = 	langNameItaliano + (langName != langNameItaliano ? ' (' + langName + ')' : '');
					} else {
						languageDisplayTitle = langName;
					}
	...
	function show() {
        ...
        /* lpnrmw substituted by following paragraph, so that all commentaries are visible when menu opened
		if (filter.val() != '') {
			filter.val('');
			filterVersions();
		}
        */
        ...
        // lpnrmw following paragraph substitutes the preceding 
		if (text_type != 'bible') {
			filter.val('');
			filterVersions();
        }
