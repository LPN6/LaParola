...
var TextComparisonWindow = function(id, parent, init_data) {
	...
	var header = $('<div class="window-header" >' +
						...
						'<input type="text" class="app-input comparison-texts" value="NR, ND" />' + // lpnrmw
						// '<input type="button" value="GO" data-i18n="[value]windows.comparison.button" class="comparison-button header-button i18n" />' +
						'<input type="button" value="VAI" data-i18n="[value]windows.comparison.button" class="comparison-button header-button i18n" />' + // lpnrmw
	...
	function doComparison() {
		...
		if (typeof reference.toSection == 'undefined') {
    		isComparing = false; // lpnrmw
			return;
		}
		//inputFragment.val( reference.toString() ); // lpnrmw
...
sofia.initMethods.push(function() {
	...
	sofia.windowTypes.push( {
		...
		init: {
			textids: 'NR, CEI1974', // lpnrmw
			fragmentid: 'Gen 1:1-2' // lpnrmw
		}
	});
});
