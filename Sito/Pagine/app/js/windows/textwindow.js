	...
	navui
		...
		.on('keypress', function(e) {
			if (e.keyCode == 13) {
				var
					userinput = navui.val(),
					//bibleref = new bible.Reference(userinput); lpnrmw
					bibleref = new bible.Reference(userinput, currentTextInfo.lang); // lpnrmw
//console.log(bibleref, currentTextInfo);
				var fragmentid = (bibleref.toSection) ? bibleref.toSection() : '',
				...
				if (sectionid != '') {
					...
					navui.val(bibleref.toString()); // lpnrmw added ;
//                    console.log("a"+bibleref.toString());
					navui[0].blur();
				}
			}
		});
	...
	textNavigator.on('change', function (e) {
		//console.log('scrollerapp:navigator:change', e.data.target);
		...
		// load new content
//		scroller.load('text', e.data.sectionid); lpnrmw
		scroller.load('text', e.data.sectionid, e.data.sectionid); // lpnrmw
	});
...
//            console.log("b"+currentLocationInfo.label);
			navui
...
//		textlistui.html('Version');
		navui.html('Riferimento').val('Riferimento');
		textlistui.html('Versione');
