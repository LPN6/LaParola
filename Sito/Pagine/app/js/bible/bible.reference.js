...
bible.parseReference = function (textReference, language) {
	...
	var
		...
		name,
		possibleMatch,
//		shortCodeRegex = /^\w{2}\d{1,3}(_\d{1,3})?$/; // lpnrmw
		shortCodeRegex = /^[a-zA-Z]\w\d{1,3}(_\d{1,3})?$/; // lpnrmw otherwise typing the reference 2P2 matches, but it is not a short code
...
// lpnrmw next section added to cater for principal references in Italian and English
//console.log(language);
if (language!=null) {
	for (bookid in bible.BOOK_DATA) {
        if (bible.BOOK_DATA[bookid].names[language]!=null) {
			for (var i=0, il=bible.BOOK_DATA[bookid].names[language].length; i<il; i++) {
				name = new String(bible.BOOK_DATA[bookid].names[language][i]).toLowerCase();
...
//console.log(matchingbookid,language);
if (matchingbookid == null) {
...
// rmw lpn usare this.lang e this.language
            var lingua = this.lang
            if (typeof lingua == "undefined") lingua = this.language
//console.log(this.lang, this.language, lingua);
			var bookName = '',
				bookNames = bible.BOOK_DATA[this.bookid].names[lingua];
//console.log(this.language,this);
			if (typeof bookNames != 'undefined') {
				bookName = bookNames[0];
			} else {
