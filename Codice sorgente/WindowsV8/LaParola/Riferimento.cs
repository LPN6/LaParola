...
namespace LaParola
{
    ...
    /// </remarks>
    public class Riferimento : IComparer
    {
        #region proprietà

        private bool versetti = true;
        /// <summary>
        /// Se il riferimento contiene un elenco di versetti (l'alternativa è un elenco di note).
        /// </summary>
        public bool Versetti
        {
            get => versetti; set => versetti = value;
        }

        private readonly List<byte[]> brani = [];
        /// <summary>
        /// Una collezione di brani, ognuno nel formato di un array di sei byte. I primi tre interi danno il versetto di inizio (numero del libro, capitolo, versetto) e i secondo tre interi l'ultimo versetto (numero del libro, capitolo, versetto).
        /// </summary>
        public Collection<byte[]> Brani
        {
            get { return new Collection<byte[]>(brani); }
            //            set { brani = value; }
        }

        private readonly List<string> note = [];
        /// <summary>
        /// Una collezione di titoli di note.
        /// </summary>
        public Collection<string> Note
        {
            get { return new Collection<string>(note); }
        }

        /// <summary>
        /// Specifica il numero di alcune parole nel primo versetto del brano.
        /// </summary>
        internal List<List<UInt16>> numeroParola = [];

        private bool daTradurre=false;
        /// <summary>
        /// Se è necessario tradurre il riferimento allo schema dei riferimenti di altre versioni.
        /// </summary>
        public bool DaTradurre
        {
            get => daTradurre; set => daTradurre = value;
        }
        ...
        /// <summary>
        /// Il costruttore della classe, che crea un riferimento a versetti senza un brano.
        /// </summary>
        public Riferimento()
        {
            //
        ...
        public Riferimento(byte libro, byte capitolo, byte versetto)
        {
            brani.Add([libro, capitolo, versetto, libro, capitolo, versetto]);
            numeroParola.Add([]);
        }
        ...
        public Riferimento(Riferimento riferimento)
        {
            ...
            if (versetti)
            {
                ...
                {
                    brani.Add([riferimento.Brani[i][0], riferimento.Brani[i][1], riferimento.Brani[i][2], riferimento.Brani[i][3], riferimento.Brani[i][4], riferimento.Brani[i][5]]);
                    numeroParola.Add([]);
                    for (int j = 0; j < riferimento.numeroParola[i].Count; ++j)
                    {
                        numeroParola[i].Add(riferimento.numeroParola[i][j]);
                    }
                }
            }
            else
        ...
        public void AggiungiBrano8Byte(byte[] brano)
        {
            brani.Add([brano[0], brano[1], brano[2], brano[4], brano[5], brano[6]]);
        ...
        public void Rimuovi(int numero)
        {
            if (numero >= 0 && numero < Count)
            {
                if (versetti)
                {
                    brani.RemoveAt(numero);
                }
                else
                {
                    note.RemoveAt(numero);
                }
            }
        }
        ...
        public bool PrimoVersettoUguale(int primoIndice, int secondoIndice)
        {
        ...
        public int Count
        {
            get { return versetti ? brani.Count : note.Count; }
        }
        ...
        public void AggiungiBrano(byte[] brano)
        {
            brani.Add(brano);
            numeroParola.Add([]);
        }
        ...
        public string ComeNotaTuttoRiferimento()
        {
            ...
            if (brani.Count > 0)
            {
                StringBuilder comeNota = new(26 * Brani.Count);
        ...
        private string ComeNotaUnBrano(int numeroBrano)
        {
            ...
            {
                notaStringa = string.Concat(notaStringa.AsSpan()[..3], "000000", notaStringa.AsSpan(9, 7), "0000000000");
            }

            if (notaStringa.EndsWith("2550000", StringComparison.Ordinal)) // un riferimento per tutto il capitolo
            {
                notaStringa = string.Concat(notaStringa.AsSpan()[..6], "000", notaStringa.AsSpan(9, 10), "0000000");
            }
