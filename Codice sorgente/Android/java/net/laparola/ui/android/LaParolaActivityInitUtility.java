...
public class LaParolaActivityInitUtility {
		...
		if (installataAggiornabile || LaParolaBrowser.getNomiVersioni().length == 0) {
			parent.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					FrameLayout activityLayout = (FrameLayout)parent.findViewById(R.id.main_activity_container);
			...
			Thread resourceCopyingThread = new Thread(new Runnable() {
				@Override
				public void run() {
					...
					parent.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							checkBibleFromResourceCopied(path);
						}
					});
				}
			});
			...
			parent.runOnUiThread(new Runnable() {
				...
				public void run() {
					if (!parent.isFinishing()) {
						HoloDialog d = new MessageDialog(parent, R.string.error, R.string.no_bible_installed);
				...
				public void publish(long progress, long size) {
					...
					if (lastPercent != percent && milliTime > lastPercentTime + 1000) {
						parent.runOnUiThread(new Runnable() {
							@Override
							public void run() {
								String command = "publishProgress(" + String.valueOf(percent) + ");";
	...
	private void initThread() {
		...
		if (checkStoragePath(LaParolaPreferences.writeStoragePath) == false) {
			if (!parent.isFinishing()) {
				parent.runOnUiThread(new Runnable() {
					@Override
					public void run() {
						HoloDialog d = new MessageDialog(parent, R.string.error, R.string.storage_error);
						d.setOnDismissListener(new OnDismissListener() {
							@Override
							public void onDismiss(DialogInterface dialog) {
								parent.finish();
	...
	public static void caricaFileDati(String[] storagePaths) {
		...
		// Log.d("LaParola", LaParolaPreferences.writeStoragePath);
		// Log.d("LaParola", LaParolaPreferences.internalStoragePath);
	...
	public static void aggiungiTesti(String storagePath, final Activity parent) {
		...
		ArrayList<String> falliti = new ArrayList<String>();
		ArrayList<String> cartelleAggiunte = new ArrayList<String>();
		...
		if (falliti.size() > 0) {
			...
			Log.d("Laparola", parent.getResources().getString(R.string.error_book_already_loaded, sb));
			/*
			parent.runOnUiThread(new Runnable() {
				@Override
				public void run() {
	...
	private static boolean stessaCartella(String path1, String path2) {
		...
		int i = 0;
		while (true) {
			tmp = "laparola" + String.valueOf(i) + ".tmp";
