...
public class BibleView extends WebView implements LaParolaBrowserClient {
	...
	public void cancelTouch() {
		// serve ad evitare il longpress quando si fa lo zoom multitouch
		// cancelLongPress non basta perché comunque il rettangolo di
		// selezione viene mantenuto

		try {
			// Si, lo so, non si accede ai metodi privati...
			...
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
				...
				w = WebView.class.getDeclaredMethod("getWebViewProvider").invoke(this);
			} else {
				// non serve, e comunque non esiste
				w = null;
			}

			if (w != null) {
				Method m = w.getClass().getDeclaredMethod("cancelTouch");
				m.setAccessible(true);
				m.invoke(w);
			}

		} catch (Exception e) {
			// Non funziona del tutto (non toglie il rettangolo), ma dovrebbe essere eseguito solo per
			// Android < 2.1 (non multitouch, e comunque non serve) ed eventualmente > 4.3...

			this.cancelLongPress();

			// togliendo il commento manda un annulla anche allo scalegesturedetector
			// forse si potrebbe risolvere mettendo speciali coordinate da
			// ignorare in scalegesturedetector...
			// ci vediamo quando uscirà android 4.x (ed il nuovo webviewprovider di
			// chrome, che magari non ha bisogno del cancel)

			// MotionEvent nev = MotionEvent.obtain(ev);
			// nev.setAction(MotionEvent.ACTION_CANCEL);
			// super.dispatchTouchEvent(nev);
		}
	}
	...
	protected void onCreateContextMenu(ContextMenu menu) {
		...
		if (result.getType() == HitTestResult.SRC_ANCHOR_TYPE) {
			final int ID_APRI = 0;
			final int ID_APRI_NUOVO_PANNELLO = 1;
			final int ID_APRI_PANNELLO_ESISTENTE = 2;
			final int ID_APRI_POPUP = 3;
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				...
				executeJavascript(String.format("checkLongTouchOnBackground(%s, %s)", sx, sy));
			} else {
				// la selezione del testo è dal menu e non parte in automatico
				mParentActivity.createBibleViewContextMenu(menu);
			}
		}
	}
	...
	public void selectAndCopyText() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			try {
				Method m = WebView.class.getDeclaredMethod("emulateShiftHeld");
				m.setAccessible(true);
				m.invoke(this);
			} catch (Exception e) {
				// fallback
				KeyEvent shiftPressEvent = new KeyEvent(0, 0, KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_SHIFT_LEFT, 0, 0);
				shiftPressEvent.dispatch(this, new DispatcherState(), this);
			}
		}
	}
	...
	public void onRichiestaIniziata(final LaParolaUrl url) {
		post(new Runnable() {
			@Override
			public void run() {
				//Log.d("laparola", "Iniziato caricamento di " + url.getUrl());
				mLoading = true;
	...
	public void setTextZoom(int zoom, boolean showToast) {
		...
		if (zoom != mTextZoom) {
			mTextZoom = zoom;
			executeJavascript("if (document.body != null) {document.body.style.fontSize = '" + String.valueOf(mTextZoom) + "%';}");
	...
	public CharSequence getAggiuntaHeader(LaParolaUrl url) {
		...
		sb.append("<link rel='stylesheet' type='text/css' href='");sb.append(assets_path);sb.append("laparola.css'>\n");
		sb.append("<script type='text/javascript' src='");sb.append(assets_path);sb.append("utils.js'></script>\n");
			...
			if (urlCorrente != null) {
				...
				boolean greek = (informazioniTesto != null) && (
						(informazioniTesto.getLingua().contains("el") && !informazioniTesto.getLingua().contains("transliterated")) ||
								(informazioniTesto.getLingua().length() == 0 && informazioniTesto.getTitolo().contains("nterlin") && informazioniTesto.getTitolo().contains("reco"))
				);

				if (!mLaParolaBrowser.inHome() && greek) {
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
						if (!InstallGentiumHelper.isInstalled(mParentActivity)) {
						...
						applyCSS(
								"@font-face {" +
										...
										"	src: url(\"file://" + InstallGentiumHelper.getFontPath(mParentActivity) + "\");" +
										"   font-style: normal;" +
										"}" +
										"@font-face {" +
										"	font-family: \"FontGreco\";" +
										"	src: url(\"file://" + InstallGentiumHelper.getFontPathItalics(mParentActivity) + "\");" +
										"   font-style: italic;" +
										"}" +
						...
						Toast.makeText(mContext, R.string.greek_not_supported, Toast.LENGTH_LONG).show();
					}
				}
			}
