...
import net.laparola.ui.android.lpnspinner.LpnAbsSpinner.SpinnerPopup;
...
public abstract class LpnDropdownPopup extends LpnPopupWindow implements SpinnerPopup {
    ...
    protected LpnHijackFocusListView mDropDownList;
	protected LpnAbsSpinner mLpnSpinner;
	...
	public LpnDropdownPopup(Context context, AttributeSet attrs, int defStyleAttr, LpnAbsSpinner lpnSpinner) {
		super(context, attrs, defStyleAttr);
		mLpnSpinner = lpnSpinner;

        setAnchorView(mLpnSpinner);
        setModal(true);
        setPromptPosition(POSITION_PROMPT_ABOVE);
        setOnItemClickListener((parent, v, position, id) -> {
            //mAdapter.sendChanged();
            mLpnSpinner.setSelection(position);
            dismiss();
            mLpnSpinner.requestLayout();
            mLpnSpinner.invalidate();
            mLpnSpinner.selectionChanged();
        });
    }
    ...
    public void show() {
        ...
        final int spinnerPaddingLeft = mLpnSpinner.getPaddingLeft();
        final int spinnerPaddingRight = mLpnSpinner.getPaddingRight();
        final int spinnerWidth = mLpnSpinner.getWidth();
        
        if (mDropDownWidth == WRAP_CONTENT) {
            setContentWidth(Math.max(
            		mLpnSpinner.measureContentWidth((SpinnerAdapter) mAdapter, mLpnSpinner.getBackground()),
                    Math.max(spinnerWidth - spinnerPaddingLeft - spinnerPaddingRight,
                    mLpnSpinner.getMinimumPopupWidth())));
        ...
        final Drawable background = mLpnSpinner.getBackground();
        int bgOffset = 0;
        if (background != null) {
        ...
        setChoiceMode();
        if (!wasShown)
            mDropDownList.setSelection(mLpnSpinner.getSelectedItemPosition());
        //mLpnSpinner.setSelection(mLpnSpinner.getSelectedItemPosition());
    }
    ...
    public void clearListSelection() {
        final LpnHijackFocusListView list = mDropDownList;
        if (list != null) {
	    ...
	    if (mDropDownList == null) {
	        ...
	        mDropDownList.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
	            public void onItemSelected(AdapterView<?> parent, View view,
	                ...
	                if (position != -1) {
	                	LpnHijackFocusListView dropDownList = mDropDownList;
	...
	protected abstract LpnHijackFocusListView createListView(Context context, boolean hijackfocus);
}