...
public class LaParolaFragment extends Fragment implements OnBibleViewListener {
    public static class AnchorAndText {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mSyncPanelsRunnable = () -> parent.syncPanels(LaParolaFragment.this);
    ...
    public void onFinishedLoading(final BibleView view) {
        if (!parent.hasFinishedLoading()) {
            ...
            // l'action bar
            view.postDelayed(() -> onFinishedLoading(view), 100);
            return;
        }
    ...
    public List<AnchorAndText> getPlainTextWithAnchors() {
        ...
        final Spanned html;
        if (testoCorrente != null) {
            m = rcomment.matcher(testoCorrente);
            testoCorrente = m.replaceAll("");
            html = Html.fromHtml(testoCorrente.toString(), Html.FROM_HTML_MODE_LEGACY);
        } else {
            html = Html.fromHtml("", Html.FROM_HTML_MODE_LEGACY);
        }

        List<AnchorAndText> res = new ArrayList<>();
        ...
        Arrays.sort(spans, Comparator.comparingInt((ClickableSpan o) -> html.getSpanStart(o)));
        ...
        for (URLSpan s : spans) {
            ...
            Timber.tag("laparola").d(t);
            String a = html.subSequence(start, end).toString();
            Timber.tag("laparola").d(a);
