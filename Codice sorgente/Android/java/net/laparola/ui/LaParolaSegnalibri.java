...
public class LaParolaSegnalibri {
	public static class GruppoSegnalibri implements Comparable<GruppoSegnalibri> {
		...
		public GruppoSegnalibri() {
			segnalibri = new ArrayList<LaParolaSegnalibri.Segnalibro>();
		}
	...
	private class SegnalibriHandler extends DefaultHandler {
		...
		private StringBuilder caratteri = new StringBuilder();
		...
		public void endElement(String namespaceURI, String localName, String qName) {
			...
			if (localName.equals("gruppo")) {
				inGruppo = false;
			} else if (localName.equals("sottogruppo")) {
				inSottoGruppo = false;
				ultimoNomeSottoGruppo = null;
			} else if (localName.equals("segnalibro")) {
				inSegnalibro = false;
				...
				inDescrizione = false;
			} else if (localName.equals("riferimento")) {
				inRiferimento = false;
			}
		}
		...
		public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
			if (localName.equals("gruppo")) {
				...
				inRiferimento = true;
                ultimiRiferimenti = new ArrayList<String>();
			}
		}
	}
	...
	public static void appendLinkGruppo(StringBuilder res, LaParolaSegnalibri.GruppoSegnalibri gruppo) {
		...
		res.append(gruppo.nome);
		res.append("</a>");
		if (gruppo.descrizione != null && gruppo.descrizione.length() > 0) {
			res.append("\n<br />");
	...
	/* package */LaParolaSegnalibri() {
		gruppi = new ArrayList<LaParolaSegnalibri.GruppoSegnalibri>();
	}
	...
	public CharSequence getPaginaGruppo(String gruppo, String versionePredefinita, LaParolaBrowser browser) {
        ...
        if (gruppi.size() == 0 ||(gruppi.size() == 1 && gruppi.get(0).segnalibri.size() == 0)) {
            ...
            for (LaParolaSegnalibri.GruppoSegnalibri g : gruppi) {
                if (gruppo == null || g.nome.equals(gruppo)) {
                    ...
                    if (g.descrizione != null && g.descrizione.length() > 0) {
                        res.append("<p>");
