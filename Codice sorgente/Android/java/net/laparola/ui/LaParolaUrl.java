...
public class LaParolaUrl {
	...
	private void init(String versionePredefinita, String versioneCommentarioPredefinita) {
        ...
        if (versione.toLowerCase(Locale.ENGLISH).equals("*bibbia")) {
            ...
            if (versioneCommentario == null || versioneCommentario.length() == 0)
        ...
        if (ancoraggio == null || ancoraggio.length() == 0)
			ancoraggio = "inizio";

		if (schema.equals("laparola")) {
		...
		} else if (schema.equals("lpfile")) {
			cacheAttiva = false;
		} else if (schema.equals("lpcomando")) {
			cacheAttiva = false;
		} else if (schema.equals("lpsegnalibri")) {
			cacheAttiva = false;
		} else if (schema.equals("lpsegnalibro")) {
			cacheAttiva = false;
		} else if (schema.equals("lppreferiti")) {
			cacheAttiva = false;
		} else if (schema.equals("lpcronologia")) {
			cacheAttiva = false;
		} else if (schema.equals("lpevidenziati")) {
			cacheAttiva = false;
		} else if (schema.equals("null")) {
			gestito = false;
		} else {
			gestito = false;
		...
		synchronized (LaParolaBrowser.mTesti) {
			try {
				if (!LaParolaBrowser.mTesti.getInfo(versione).getTipo().contains(TestoTipi.BIBBIA)) {
					...
					if (versioneDaControllare.equals(""))
						return null;
				}
			...
			if (v2 >= versettiInUltimoCapitolo) {
	            ...
	            rif = getUrlLCV(l1, c1, 1, l2, c2, 255,
                        String.format("%s_%d_%d", LaParolaBrowser.mTesti.getLibroAbbreviazioneUsata(l2), c2, v2 + 1));
			}
	...
	public boolean equals(Object o) {
		if (!(o instanceof LaParolaUrl)) {
			return false;
		}
		LaParolaUrl other = (LaParolaUrl) o;
	...
	public EnumSet<TestoTipi> getTipoTesto() {
		try {
			return LaParolaBrowser.getInformazioniTesto(versione).getTipo();
		} catch (Exception e) {
