...
//#NB
//@ServiceProvider(service = TestiAPI.class)
//public class Testi extends Object implements Closeable, TestiAPI {
//#eclipse

public class Testi extends Object implements Closeable {
	...
	// public static final String LIBRI_NOMI_INGLESE =
	...
	// private static final String LIBRI_ABBREVIAZIONI_USATE_ENGLISH =
	// "|Gen|Ex|Le|Nu|De|Josh|Judg|Ru|1Sam|2Sam|1K|2K|1Chr|2Chr|Ezra|Ne|Tob|Judi|Est|1M|2M|Job|Ps|Prov|Ec|SS|Wis|Sir|Is|Jer|Lam|Bar|Ezek|Dan|Hos|Joel|Am|Ob|Jon|Mi|Na|Hab|Zep|Hag|Zec|Mal|Mat|Mar|Lu|John|Ac|Ro|1Co|2Co|Ga|Eph|Phili|Col|1Th|2Th|1Ti|2Ti|Tit|Phile|Heb|Jam|1P|2P|1J|2J|3J|Jude|Rev";
	...
	// private static final String LibriAbbreviazioniRiconosciuteInglese =
	// "|gen,gn|ex|le,lv|nm,nu|de,dt|jos,js|judg,jg|rt,ru|1 s,1s,isam|2 s,2s,iis|1 k,1k,ik|2 k,2k,iik|1 ch,1ch,ich|2 ch,2ch,iich|ezr|ne|tb,to|jdt,jt,judi|est,et|1 m,1m,im|2 m,2m,iim|jb,job|ps|pr,pv|ec|so,ss|w|si|is|je,jr|la|b|ez|da,dn|ho|jl,joe|am|o|jon|mi|na|hab|zep|hag|zec|mal,ml|mat,mt|mar,mk,mr|lk,lu|jn,joh|ac|rm,ro|1 co,1co,ico|2 co,2co,iico|ga|ep|phi,php,pl|cl,co|1 th,1th,1ts,ith|2 th,2th,2ts,iith|1 ti,1ti,1tm,iti|2 ti,2ti,2tm,iiti|ti,tt|phile,phlm,phm,pm|he|jam,jas,jm|1 p,1p,ip|2 p,2p,iip|1 j,1j,ij|2 j,2j,iij|3 j,3j,iiij|jd,jude|re";
	...
	private void costruttore(int versioneProgramma1, int versioneProgramma2) {
		listaVersioni = new HashMap<String, Testo>();
		listaFileIllegibili = new HashSet<String>();
		...
		for (int i = 1; i <= 73; ++i) {
			...
			for (int j = 0; j < abbreviazioni.length; ++j) {
				libriAbbreviazioniRiconosciute.put(abbreviazioni[j], i);
			}
		}
	...
	public boolean aggiungiTesto(String percorso) {
		...
		try {
			...
			if (listaVersioni.containsKey(nome)) {
				t.chiudi();
				return false;
			}
			listaVersioni.put(nome, t);
			ultimaBibbia = nome;
			if (t.capitoliInLibro[17] > 0)
				ultimaBibbiaCompleta = nome;
			if (listaFileIllegibili.contains(percorso))
	...
	public List<String> aggiungiTestiDaDirectory(String directory) {
		ArrayList<String> giaCaricati = new ArrayList<String>();
		String directoryModificata = directory;
		File dir = new File(directoryModificata);

		FilenameFilter filter = new FilenameFilter() {
			@Override
			public boolean accept(File directoryDaControllare, String name) {
				return name.endsWith(".lpj");
			}
		};
		...
		if (!directoryModificata.equals(""))
			directoryModificata += File.separator;

		for (int i = 0; i < children.length; i++) {
			String nomeFile = directoryModificata + children[i];
	...
	@Override
	public void close() {
		Iterator<Testo> it = listaVersioni.values().iterator();
		while (it.hasNext()) {
			it.next().chiudi();
		}
	}
	...
	public String[] nomiVersioni() {
		String[] nomi = new String[listaVersioni.size()];

		Iterator<String> it = listaVersioni.keySet().iterator();
		int i = 0;
		while (it.hasNext()) {
			nomi[i] = it.next();
			i += 1;
	...
	// public String getLinguaTesto(String nomeVersione) throws TestoNonEsisteException {
	// return getInfo(nomeVersione).getLingua();
	// }

	public String getUltimaBibbia() {
		return (ultimaBibbiaCompleta.equals("") ? ultimaBibbia : ultimaBibbiaCompleta);
	}
	...
	public void interrompiGetBrano(String nomeVersione) {
		try {
			getTesto(nomeVersione).setInterrompiGetBrano(true);
		} catch (TestoNonEsisteException e) {
	...
	public List<String> note(String nomeVersione) throws TestoNonEsisteException {
		List<String> note = new ArrayList<String>(getTesto(nomeVersione).noteTitoli.size());
	...
	public List<String> noteConTitolo(String nomeVersione) throws TestoNonEsisteException {
		...
		List<String> note = new ArrayList<String>(numeroNote);
								...
								if (inFrase) {
									...
									if (differenze[0] < -differenze[1]) // cioè string.Compare(notaOccorrenze, notaRiferimenti) < 0 per due note quando una non è ad un brano
									{
										...
										if (differenze[0] > differenze[1]) // cioè string.Compare(notaOccorrenze, notaRiferimenti) > 0 per due note quando una non è ad un brano
										{
											...
											if (tipoOperazione.length() == 1) {
												if (occorrenzeInBrano.getNote().size() > 0
	...
	private int[] calcolaDifferenzeDelleNote(int primoCarattereComeNumero, String notaRiferimenti, String notaOccorrenze) {
		...
		int[] differenze = { differenzaVersetti, differenzaRicercata };
		return differenze;
	}
	...
	public Riferimento convertiDaStandard(Riferimento riferimento, String nomeVersione) {
		...
		for (int[] branoDaConvertire : rif.getBrani()) {
			...
			} catch (TestoNonEsisteException e) { // "nomeVersione" non esiste; non cambiamo il riferimento
				return rif;
			}
		}
	...
	public String convertiRiferimentoDa3Numeri(String riferimentoDaConvertire) {
		...
		if (!riferimentoDaConvertire.equals("")) {
	...
	public Riferimento convertiRiferimento(String riferimento) {
		...
		if (riferimentoModificato.equals("")) {
		...
		do {
			...
			if (riferimentoBrano[0] == 0 && !riferimentoDaAnalizzare.equals("") && (!Character.isLetter(riferimentoDaAnalizzare.charAt(0)))) {
				if (riferimentoDaAnalizzare.indexOf(':') == -1 && riferimentoDaAnalizzare.indexOf('.') == -1 && !versettoMancante) {
					riferimentoDaAnalizzare = Integer.toString(capitolo) + ":" + riferimentoDaAnalizzare;
				}
			...
			if (riferimentoBrano[0] > 0) {
				...
				if (riferimentoDaAnalizzare.indexOf(':') == -1 && riferimentoDaAnalizzare.indexOf('.') == -1) {
					...
					if (trattino) {
						if (!riferimentoModificato.equals("") && (!Character.isLetter(riferimentoModificato.charAt(0)))
		...
		} while (!riferimentoModificato.equals(""));
		return nuovoRiferimento;
	}
	...
	private int[] convertiRiferimentoDaTestoA4Byte(String riferimentoTestuale, boolean primaDelTrattino) {
		...
		if (primaNonLettera == riferimento.length() - 1 && Character.isLetter(riferimento.charAt(riferimento.length() - 1))) {
			...
			for (int j = 0; j < riferimentoRimanente.length() && Character.isDigit(riferimentoRimanente.charAt(j)); ++j) {
				capitoloNumerico.append(riferimentoRimanente.substring(j, j + 1));
			}
			try {
		...
		if (!riferimentoRimanente.equals("")) {
			...
			if ((formato.getRiferimentoTipo() == RiferimentoTipo.VIRGOLA || formato.getRiferimentoTipo() == RiferimentoTipo.CITAZIONE)
					&& (posDivisoreCapitoloVersetto == -1 || (riferimentoRimanente.indexOf(",") < posDivisoreCapitoloVersetto && riferimentoRimanente.indexOf(",") >= 0))) {
				posDivisoreCapitoloVersetto = riferimentoRimanente.indexOf(",");
			}
			...
			for (int j = 0; j < riferimentoRimanente.length() && Character.isDigit(riferimentoRimanente.charAt(j)); ++j) {
				versettoNumerico.append(riferimentoRimanente.substring(j, j + 1));
			}
			try {
		...
		if (!riferimentoRimanente.equals("")) {
			...
			for (int j = 0; j < riferimentoRimanente.length() && Character.isDigit(riferimentoRimanente.charAt(j)); ++j) {
				parolaNumerico.append(riferimentoRimanente.substring(j, j + 1));
			}
			try {
	...
	public String normalizzaRiferimento(Riferimento riferimento, RiferimentoFormato formatoDelRiferimento) {
		...
		if (formatoDelRiferimento != RiferimentoFormato.NESSUNO && riferimento.getVersetti()) { // se è un riferimento con note, restituisce niente
			...
			for (int i = 0; i < nRiferimenti; ++i) {
				...
				if (!riferimentoNormalizzato.equals("")) {
		...
		if (formato.getRiferimentoTipo() == RiferimentoTipo.CITAZIONE && !riferimentoNormalizzato.equals(""))
			riferimentoNormalizzato += ":";
	...
	private int versettiFinoACapitolo(int libro, int capitolo) {
		...
		return (versione.equals("") ? 0 : listaVersioni.get(versione).indiceCapitoli[listaVersioni.get(versione).indiceLibri[libro - 1] + capitolo]);
	}
	...
	public int libroDiCapitolo(int capitolo, String nomeVersione) {
		int capitoloPos = (capitolo < 1 ? 1 : capitolo);
		int libro = 0;
		do {
			libro++;
	...
	public Riferimento riferimentoDiVersetto(int versetto, String nomeVersione) {
		int versettoPos = (versetto < 1 ? 1 : versetto);
	...
	public String convertiTitoloNotaARiferimento(String notaDaConvertire) {
		...
		for (String nota : note) {
			try {
				if (riferimento.toString().length() > 0)
					riferimento.append(";");
	...
	public List<ComponenteInformazioni> getTestiDisponibili(String url) throws ParserConfigurationException, IOException, SAXException {
		...
		List<ComponenteInformazioni> listaComponenti = new ArrayList<ComponenteInformazioni>();
		Map<String, Boolean> versioniAggiornabili = new HashMap<String, Boolean>();
		cacheUltimoFileAggiornamenti = false;

		Iterator<String> it1 = listaVersioni.keySet().iterator();
		while (it1.hasNext())
			versioniAggiornabili.put(it1.next(), false);
		...
		if (url != null) {
			...
			for (int i = 0; i < nList.getLength(); ++i) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (tipoStringa.equals("bibbia"))
						tipo = TestoTipi.BIBBIA;
					else if (tipoStringa.equals("commentario"))
						tipo = TestoTipi.COMMENTARIO;
					else if (tipoStringa.equals("dizionario"))
						tipo = TestoTipi.DIZIONARIO;
					else if (tipoStringa.equals("libro"))
						tipo = TestoTipi.LIBRO;
					else
						tipo = TestoTipi.NESSUNO;
					...
					Iterator<String> it = listaVersioni.keySet().iterator();
					while (it.hasNext()) {
						nomeVersioneInstallata = it.next();
		...
		while (it2.hasNext()) {
			...
			if (!versioniAggiornabili.get(versioneNonAggiornabile)) {
				ComponenteInformazioni ci = null;
				try {
